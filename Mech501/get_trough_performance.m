function [L, Tf_store, Qdot_trough_store, rec_eff_store, fdarcy_store] = get_trough_performance(Ti,To,mdot,L,dL,QdotS,muf,cpf,kf,rough,r1,r2,D1,D2,k_al,alpha_e,ve,ke,Be,Te,eps_a,eps_s,verbose,plot_trough)
...
while Tf_i < To
    ...
    [~, ~, ~, rec_eff, ~, fdarcy] = parabolic_trough_HT_model(mdot,QdotS,muf,cpf,kf,rough,r1,r2,D1,D2,k_al,alpha_e,ve,ke,Be,Te,Tf,eps_a,eps_s,verbose);
    Qdot_trough_i = QdotS*rec_eff; %Energy collected by one metre of trough
    dT = (Qdot_trough_i*L)/(mdot*cpf); %Get change in temperature over this metre of trough
