function mit18086_navierstokes_MovingObs
...
xObs = 0.5; yObs = 0.5;   %Obstacle position
eta = 10^(-3);
...
figure;
for k = 1:nt
    %Define Obstacle with time-dep position
    xObsT = xObs + 0.25*sin(k*2*pi/nt);
    yObsT = yObs;
    %yObsT = xObs + 0.25*cos(k*2*pi/nt);
    switch lower(obstacle)
        case {'circle'}
            circ = @(x, y) sqrt(x.^2 + y.^2);
            phi = @(x, y) circ(x-xObsT, y-yObsT) - 0.15;
            phip = phi(X, Y);
       
            %Smoothed Indicator Function
            sig = 1*10^(-1);
            gauss = @(x, y, sig) exp(-(x.^2 + y.^2)./(sig.^2)); % gaussian centered at 0
            %Not sure why but yObsT needs to go with mod(X,lx) and vice
            %versa for phip to track indFunc's movements
            indFunc = gauss(mod(X, lx)-yObsT, mod(Y, ly)-xObsT, sig);
   ...
   %U = dt.*(uxx-indFunc.*u./eta) + u
   %V = dt.*(vxx-indFunc.*v./eta) + v
   U = U-dt*(UVy(2:end-1,:)+U2x + indFunc(2:end-1,1:end-1).*U./eta);
   V = V-dt*(UVx(:,2:end-1)+V2y + indFunc(1:end-1,2:end-1).*V./eta);
   ...
   if k==1||floor(nsteps*k/nt)>floor(nsteps*(k-1)/nt)
      ...
      contour(X, Y, phip, [0 0], 'm', 'LineWidth', 2)
